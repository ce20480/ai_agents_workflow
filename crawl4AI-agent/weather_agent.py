...
async def get_lat_lng(
    ...
    async with ctx.deps.geo_semaphore:
        params = {
...
async def get_weather(ctx: RunContext[Deps], lat: float, lng: float) -> dict[str, Any]:
    ...
    # Use semaphore for rate limiting
    async with ctx.deps.weather_semaphore:
        params = {
...
async def main():
    async with AsyncClient() as client:
        ...
        # Allow only 1 request at a time for geocoding and add delay between requests
        geo_semaphore = asyncio.Semaphore(1)
        # Allow 2 concurrent requests for weather API
        ...
        deps = Deps(
            ...
            geo_semaphore=geo_semaphore,
            weather_semaphore=weather_semaphore,
        )
